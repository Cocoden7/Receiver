...
pkt_status_code pkt_decode(const char *data, const size_t len, pkt_t *pkt)
{
    ...
    // A faire : CRC32 etc.
    //pkt_set_crc2(pkt, const uint32_t crc2)
...
{
    size_t encode_size = varuint_predict_len(val);
    // Varuint trop grand
    if(len < encode_size || encode_size == -1)
    {
        return -1;
    }
    // Varuint sur 1 byte
...
ssize_t predict_header_length(const pkt_t *pkt)
{
    if(pkt->length < 32768)
    {
        if(pkt->l == 0)
        {
            return((2+1+5+1+7+8+4)/8);
        }
        else
        {
            return((2+1+5+1+15+8+4)/8);
        }
    }
    return -1;
}
